...
// in the same function. Every statement binds s.UserID as a plain `?`
// parameter, never string-built — see ARCHITECTURE.md's multi-user section.
...
func (s *DBSyncer) upsertDailySummary(ctx context.Context, sum healthdata.DailySummary) error {
	...
	_, err := s.DB.ExecContext(ctx, `
		INSERT INTO watch_daily_summary (
			user_id, day, steps_total, distance_m, floors_climbed, altitude_gain_m, sedentary_minutes, active_minutes,
	...
	`,
		s.UserID, sum.Day, sum.StepsTotal, sum.DistanceM, sum.FloorsClimbed, sum.AltitudeGainM, sum.SedentaryMinutes, sum.ActiveMinutesTotal,
...
// rows (natural PK (user_id, day, zone_type), true upsert rather than
// delete+insert).
func (s *DBSyncer) upsertHeartRateZoneMinutes(ctx context.Context, rows []healthdata.HeartRateZoneMinutes) error {
	for _, r := range rows {
		if _, err := s.DB.ExecContext(ctx, `
			INSERT INTO watch_heart_rate_zone_minutes (user_id, day, zone_type, minutes) VALUES (?, ?, ?, ?)
			ON CONFLICT(user_id, day, zone_type) DO UPDATE SET minutes = excluded.minutes
		`, s.UserID, r.Day, r.ZoneType, r.Minutes); err != nil {
...
func (s *DBSyncer) upsertCaloriesByZone(ctx context.Context, rows []healthdata.CaloriesByZone) error {
	for _, r := range rows {
		if _, err := s.DB.ExecContext(ctx, `
			INSERT INTO watch_calories_by_zone (user_id, day, zone_type, kcal) VALUES (?, ?, ?, ?)
			ON CONFLICT(user_id, day, zone_type) DO UPDATE SET kcal = excluded.kcal
		`, s.UserID, r.Day, r.ZoneType, r.Kcal); err != nil {
...
func (s *DBSyncer) upsertHourlySteps(ctx context.Context, rows []healthdata.HourlySteps) error {
	for _, r := range rows {
		if _, err := s.DB.ExecContext(ctx, `
			INSERT INTO watch_steps_hourly (user_id, day, hour, steps, distance_m, calories) VALUES (?, ?, ?, ?, ?, ?)
		...
		`, s.UserID, r.Day, r.Hour, r.Steps, r.DistanceM, r.Calories); err != nil {
...
func (s *DBSyncer) upsertHeartRateSamples(ctx context.Context, rows []healthdata.HeartRateSample) error {
	...
	for _, r := range rows {
		if _, err := s.DB.ExecContext(ctx, `
			INSERT INTO watch_heart_rate_intraday (user_id, recorded_at, bpm, cached_at) VALUES (?, ?, ?, ?)
			ON CONFLICT(user_id, recorded_at) DO UPDATE SET bpm = excluded.bpm, cached_at = excluded.cached_at
		`, s.UserID, r.RecordedAt.Format(time.RFC3339), r.Bpm, now); err != nil {
...
func (s *DBSyncer) upsertBloodGlucoseSamples(ctx context.Context, rows []healthdata.BloodGlucoseSample) error {
	for _, r := range rows {
		if _, err := s.DB.ExecContext(ctx, `
			INSERT INTO watch_blood_glucose_sample (user_id, recorded_at, mg_dl, measurement_source, measurement_timing, meal_type, specimen)
			VALUES (?, ?, ?, ?, ?, ?, ?)
		...
		`, s.UserID, r.RecordedAt.Format(time.RFC3339), r.MgDl, r.MeasurementSource, r.MeasurementTiming, r.MealType, r.Specimen); err != nil {
...
func (s *DBSyncer) upsertCoreBodyTemperatureSamples(ctx context.Context, rows []healthdata.CoreBodyTemperatureSample) error {
	for _, r := range rows {
		if _, err := s.DB.ExecContext(ctx, `
			INSERT INTO watch_core_body_temperature_sample (user_id, recorded_at, celsius, measurement_location) VALUES (?, ?, ?, ?)
			ON CONFLICT(user_id, recorded_at) DO UPDATE SET celsius = excluded.celsius, measurement_location = excluded.measurement_location
		`, s.UserID, r.RecordedAt.Format(time.RFC3339), r.Celsius, r.MeasurementLocation); err != nil {
...
func (s *DBSyncer) upsertSleepSession(ctx context.Context, sess healthdata.SleepSession, stages []healthdata.SleepStage) (int64, error) {
	...
	err := s.DB.QueryRowContext(ctx, `
		...
		ON CONFLICT(user_id, day, start_time) DO UPDATE SET
		...
		RETURNING id
	`,
		s.UserID, sess.Day, sess.StartTime.Format(time.RFC3339), sess.EndTime.Format(time.RFC3339), isMainInt,
...
// sync_details.go's syncRespiratoryRateSleepSummary). sessionID already
// uniquely identifies a row scoped to s.UserID (it came from
// upsertSleepSession above in the same pass), but the WHERE clause still
// pins user_id defensively so this can never touch another user's row.
func (s *DBSyncer) updateSleepSessionRespiratoryRate(ctx context.Context, sessionID int64, deep, light, rem, full *float64) error {
	...
	`, deep, light, rem, full, sessionID, s.UserID)
	if err != nil {
...
func (s *DBSyncer) upsertExerciseSession(ctx context.Context, sess healthdata.ExerciseSession) error {
	_, err := s.DB.ExecContext(ctx, `
		INSERT INTO watch_exercise_session (
			user_id, day, exercise_type, start_time, end_time, duration_minutes,
	...
	`,
		s.UserID, sess.Day, sess.ExerciseType, sess.StartTime.Format(time.RFC3339), sess.EndTime.Format(time.RFC3339),
...
func (s *DBSyncer) upsertECGReading(ctx context.Context, r healthdata.ECGReading) error {
	_, err := s.DB.ExecContext(ctx, `
		INSERT INTO watch_ecg_reading (user_id, type, recorded_at, classification, avg_heart_rate_bpm)
		VALUES (?, ?, ?, ?, ?)
		ON CONFLICT(user_id, type, recorded_at) DO UPDATE SET
			classification     = excluded.classification,
			avg_heart_rate_bpm = excluded.avg_heart_rate_bpm
	`, s.UserID, r.Type, r.RecordedAt.Format(time.RFC3339), r.Classification, r.AvgHeartRateBpm)
	if err != nil {
...
func (s *DBSyncer) upsertBodyMeasurement(ctx context.Context, day string, weightKg, heightCm, bodyFatPct *float64) error {
	_, err := s.DB.ExecContext(ctx, `
		INSERT INTO body_measurement (user_id, day, weight_kg_raw, height_cm_raw, body_fat_pct_raw)
		VALUES (?, ?, ?, ?, ?)
		ON CONFLICT(user_id, day) DO UPDATE SET
	...
	`, s.UserID, day, weightKg, heightCm, bodyFatPct)
	if err != nil {
